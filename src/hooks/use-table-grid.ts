...
import { TableGridCell } from "@/components/table/table-grid/table-grid-cell";
import { useDataGridStore } from "@/store/data-grid.store";
import type {
	CellPosition,
	DataGridState,
	NavigationDirection,
	...
	SearchState,
	UpdateCell,
} from "@/types/data-grid.type";
import { getCellKey, getRowHeightValue, parseCellKey } from "@/utils/data-grid-helper";
...
export interface UseDataGridProps<TData> extends Omit<TableOptions<TData>, "pageCount" | "getCoreRowModel"> {
	...
	) => Partial<CellPosition> | Promise<Partial<CellPosition>> | null | undefined;
...
export const useTableGrid = <TData>({
	columns,
	data,
	onDataChange,
	...
	const {
		...
		isScrolling,
		store,
	} = useDataGridStore({ initialState: initialState as DataGridState, rowHeightProp });
